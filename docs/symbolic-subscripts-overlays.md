# Symbolic Subscripts and Overlay Field Lists

This document describes the K65 dialect feature for declaring packed field overlays on fixed addresses and accessing those fields with symbolic subscripts.

## Overview

An overlay declaration binds a symbol to a base address and defines a field layout on top of that address.
...
- field base = `overlay_base + field_offset`
- indexed field element address = `field_base + index * element_width`

`= <address>` on an overlay var must be a constant numeric expression.

## Diagnostics

The implementation reports hard errors for:

- missing `= <addr>` in overlay declarations
- non-constant overlay base expressions
- duplicate field names in one overlay
- invalid field counts (`<= 0` or out of range)
- unknown field access (with nearest-name suggestion)
- invalid indexing on overlay aggregate (`foo[1]`, `foo[i]`)
...
- `VarDecl` gained `overlay_fields: Option<Vec<OverlayFieldDecl>>`.
- New `OverlayFieldDecl { name, data_width, count }`.
...
- `var_decl_parser` now parses a bracket payload as either:
  ...
  - overlay field list.
- Overlay field list parsing is implemented by:
  - `parse_var_bracket_payload`
  - `parse_overlay_field_list`
  - `parse_overlay_field_entry`
...
- Overlay field `count` expressions are expanded.
...
- New metadata:
  - `OverlayFieldMeta`
  - `OverlayMeta`
  - `VarMeta.overlay: Option<OverlayMeta>`
- `eval_var_layout` handles both traditional vars and overlays.
- `eval_overlay_layout` computes field offsets, sizes, and total overlay size, with validation.

### Lowering (`crates/core/src/lower.rs`)

- Added overlay-aware resolution path:
...
- Supports printing overlay declarations and `Expr::Index`.
